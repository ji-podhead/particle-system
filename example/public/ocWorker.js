...
self.onmessage = function (input) {
  ...
  switch (input.data.task) {
    ...
    case ("updateDefaultValues"): {
      // Assuming input.data is the MessageEvent and input.data.data is the actual payload
      const payload = input.data.value.object;
      object1 = payload; // Assign the payload to object1
      const lifeTime=object1.properties.get("lifeTime")
      console.log(object1)
      console.log(lifeTime)
      // Check if 'properties' and 'lifeTime' exist and are structured as expected
      if (lifeTime) {
        object1.lifeTime = lifeTime.array.slice();
        
        console.log("lifeTimeObject")
        console.log(object1)
        
      } else {
        console.error("Properties or lifeTime property not found in the payload.");
        object1.lifeTime = []; // Default to empty array if not found             
      }
      console.log("Updated object1 with lifeTime array:", object1);
      break;
    }
    ...
    //  console.log(object1)
     ...
     function updateSimulation(delta, reset, kill) {
		...
		for (let count=object1.instanceCount;count>0;count--) {
			const index =(object1.instanceCount-count)
